 ...
 function useSwipe(onSwipeLeft, onSwipeRight, onSwipeDown) {
   const [touchStart, setTouchStart] = useState(null)
   const onTouchStart = (e) => setTouchStart({ x: e.targetTouches[0].clientX, y: e.targetTouches[0].clientY })
   const onTouchEnd = (e) => {
     if (!touchStart) return
     const deltaX = touchStart.x - e.changedTouches[0].clientX
     const deltaY = touchStart.y - e.changedTouches[0].clientY
     if (Math.abs(deltaY) > 100) { onSwipeDown(); return }
     if (Math.abs(deltaX) > 50) { deltaX > 0 ? onSwipeLeft() : onSwipeRight() }
     setTouchStart(null)
   }
 ...
 export default function ClientGallery() {
   ...
   const getDisplayPhotos = () => {
     let filtered = photos
     if (showFavs) filtered = photos.filter(p => favourites.includes(p.id))
     else if (activeSet !== 'all') filtered = photos.filter(p => p.set_id === activeSet)
     return filtered
   }
   ...
   const goNext = () => currentIndex < displayedPhotos.length - 1 && setSelected(displayedPhotos[currentIndex + 1])
   const goPrev = () => currentIndex > 0 && setSelected(displayedPhotos[currentIndex - 1])
       ...
       {gallery.cover_image && (
         <div className="relative w-full h-64 sm:h-80 md:h-[480px] overflow-hidden">
           ...
           <div className="absolute bottom-6 md:bottom-10 left-5 md:left-10 right-5">
             ...
             <h1 style={{ fontFamily: "'Playfair Display', serif" }}
               className="text-3xl sm:text-4xl md:text-6xl font-bold drop-shadow-lg">{gallery.name}</h1>
             {gallery.event_date && (
       ...
       <div className="max-w-6xl mx-auto px-4 md:px-8 py-6 md:py-10">
         ...
         <div className="flex justify-between items-center gap-3 mb-6 flex-wrap">
           <div className="flex gap-2 overflow-x-auto pb-1 flex-1">
             {/* All photos */}
             <button
               onClick={() => { setActiveSet('all'); setShowFavs(false) }}
               className={`px-3 md:px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition ${activeSet === 'all' && !showFavs ? 'bg-[#c9a84c] text-black' : 'glass text-gray-400 hover:text-white'}`}>
               All ({photos.length})
             </button>
             ...
             {sets.map(set => (
               <button key={set.id}
                 onClick={() => { setActiveSet(set.id); setShowFavs(false) }}
                 className={`px-3 md:px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition ${activeSet === set.id && !showFavs ? 'bg-[#c9a84c] text-black' : 'glass text-gray-400 hover:text-white'}`}>
             ...
             {/* Favourites */}
             <button
               onClick={() => { setShowFavs(true); setActiveSet('all') }}
               className={`px-3 md:px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition ${showFavs ? 'bg-[#c9a84c] text-black' : 'glass text-gray-400 hover:text-white'}`}>
               ❤️ ({favourites.length})
             </button>
           </div>

           <a href={`/api/download-all?galleryId=${gallery.id}&galleryName=${encodeURIComponent(gallery.name)}`}
             className="glass px-3 md:px-4 py-2 rounded-full text-xs font-semibold text-gray-300 hover:text-white transition whitespace-nowrap">
             ⬇️ Download All
           </a>
         </div>

         {/* Photos — grouped by set if viewing all */}
                 ...
                 <div key={set.id}>
                   ...
                   <div className="flex items-center gap-4 mb-4">
                     <div>
                       ...
                       {set.description && (
                         <p className="text-gray-500 text-sm mt-0.5">{set.description}</p>
                       )}
       ...
       {selected && (
         <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-50 backdrop-blur-sm"
           ...
           <button onClick={() => setSelected(null)}
             className="absolute top-4 right-4 w-10 h-10 rounded-full glass flex items-center justify-center text-white hover:text-[#c9a84c] transition z-10">
             ✕
           </button>
           {currentIndex > 0 && (
             <button onClick={goPrev}
               className="absolute left-3 md:left-6 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full glass flex items-center justify-center text-white hover:text-[#c9a84c] transition z-10">
               ←
             </button>
           )}
           ...
           {currentIndex < displayedPhotos.length - 1 && (
             <button onClick={goNext}
               className="absolute right-3 md:right-6 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full glass flex items-center justify-center text-white hover:text-[#c9a84c] transition z-10">
               →
             </button>
           )}
           ...
           <p className="absolute bottom-14 left-1/2 -translate-x-1/2 text-gray-700 text-xs md:hidden">
             Swipe to navigate
           </p>
         </div>
       )}
 ...
 function PhotoCard({ photo, isFav, onToggleFav, onClick }) {
   return (
     <div className="relative group rounded-xl overflow-hidden bg-[#111] aspect-square cursor-pointer"
       onClick={onClick}>
       <img src={`/api/photo?key=${photo.b2_key}`} alt={photo.filename}
         className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
       <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
       <button onClick={e => { e.stopPropagation(); onToggleFav() }}
         className="absolute top-2 right-2 text-lg opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-110">
         {isFav ? '❤️' : '🤍'}
       </button>
     </div>
   )
